...
def save_tokens(token_dict, json_path=TOKEN_PATH):
    with open(json_path, 'w') as f:
        json.dump(token_dict, f)

    with open('./REFRESH_TOKEN.txt', 'w') as f:
        f.write(token_dict['refresh_token'])
...
def print_auth_token_response(response):
    ...
    save_tokens(token_dict, json_path=TOKEN_PATH)


def login():
    selenium_version = int(selenium.__version__.split('.')[0])
    if selenium_version < 4:
        caps = DesiredCapabilities.CHROME.copy()
        caps["goog:loggingPrefs"] = {"performance": "ALL"}  # enable performance logs
        driver = webdriver.Chrome(CHROME_DRIVER, desired_capabilities=caps)
    else:
